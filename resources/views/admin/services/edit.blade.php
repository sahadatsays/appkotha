        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white shadow-sm rounded-lg p-6">
                    ...
                    @php
                        $stepsText = '';
                        if (is_array($service->process_steps)) {
                            $stepsText = collect($service->process_steps)->pluck('title')->implode("\n");
                        }
                    @endphp
                    <div>
                        <label for="process_steps_text" class="block text-sm font-medium text-gray-700">Steps (one per line)</label>
                        <textarea name="process_steps_text" id="process_steps_text" rows="6"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('process_steps_text', $stepsText) }}</textarea>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <div class="bg-white shadow-sm rounded-lg p-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $service->is_published) ? 'checked' : '' }}
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button type="submit" class="px-4 py-2 bg-indigo-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-indigo-700">Update Service</button>
                    </div>
                </div>
                ...
                <div class="bg-white shadow-sm rounded-lg p-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="starting_price" id="starting_price" value="{{ old('starting_price', $service->starting_price) }}" step="0.01" min="0"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div class="bg-white shadow-sm rounded-lg p-6">
                    ...
                    <div class="space-y-4">
                        @if($service->image)
                            <div class="mb-4">
                                <img src="{{ Storage::url($service->image) }}" alt="{{ $service->name }}" class="w-full h-32 object-cover rounded-lg">
                            </div>
                        @endif
                        <div>
                ...
                <div class="bg-white shadow-sm rounded-lg p-6 border border-red-200">
                    ...
                    <form action="{{ route('admin.services.destroy', $service) }}" method="POST"
                        ...
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full px-4 py-2 bg-red-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-red-700">Delete Service</button>
                    </form>
                </div>
            </div>
        </div>
