<x-admin-layout>
        ...
        <nav class="flex mb-4" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li class="flex items-center">
                    ...
                    <span class="ml-1 text-gray-700 font-medium">{{ $setting->key }}</span>
                </li>
            </ol>
        </nav>
    ...
    <form action="{{ route('admin.settings.update', $setting) }}" method="POST" enctype="multipart/form-data" x-data="{ selectedType: '{{ old('type', $setting->type) }}' }">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white shadow-sm rounded-lg p-6">
                    ...
                    <div x-show="['text', 'url', 'number'].includes(selectedType)">
                        ...
                        <input type="text" name="value" id="value_text" value="{{ old('value', $setting->value) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    ...
                    <div x-show="selectedType === 'textarea'">
                        ...
                        <textarea name="value" id="value_textarea" rows="4"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('value', $setting->value) }}</textarea>
                    </div>
                    ...
                    <div x-show="selectedType === 'color'">
                        ...
                        <div class="mt-1 flex items-center space-x-3">
                            <input type="color" name="value" id="value_color" value="{{ old('value', $setting->value ?: '#000000') }}"
                                class="h-10 w-20 rounded border-gray-300">
                            <input type="text" id="color_text" value="{{ old('value', $setting->value ?: '#000000') }}"
                    ...
                    <div x-show="selectedType === 'image'">
                        @if($setting->type === 'image' && $setting->value)
                            <div class="mb-4">
                                ...
                                <img src="{{ Storage::url($setting->value) }}" alt="" class="h-24 w-auto rounded-lg border border-gray-200">
                            </div>
                        @endif
                        <label for="value_image" class="block text-sm font-medium text-gray-700">{{ $setting->type === 'image' && $setting->value ? 'Replace Image' : 'Image' }}</label>
                    ...
                    <div x-show="selectedType === 'boolean'">
                        <div class="flex items-center">
                            <input type="checkbox" name="value" id="value_boolean" value="1" {{ old('value', $setting->value) ? 'checked' : '' }}
                    ...
                    <div x-show="selectedType === 'json'">
                        ...
                        <textarea name="value" id="value_json" rows="6"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono text-sm">{{ old('value', $setting->value) }}</textarea>
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="description" id="description" rows="2"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $setting->description) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-gray-50 shadow-sm rounded-lg p-6">
                    ...
                    <dl class="text-sm space-y-2">
                        <div>
                            ...
                            <dd class="text-gray-900">{{ $setting->created_at->format('M d, Y H:i') }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Updated</dt>
                            <dd class="text-gray-900">{{ $setting->updated_at->format('M d, Y H:i') }}</dd>
                        </div>
                    </dl>
                </div>
