<x-admin-layout>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Manage your digital products and software</p>
        </div>
    ...
    <div class="bg-white shadow-sm rounded-lg p-4 mb-6">
        <form action="{{ route('admin.products.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
            ...
            @if(request()->hasAny(['search', 'status']))
                <a href="{{ route('admin.products.index') }}" class="inline-flex items-center px-4 py-2 text-xs text-gray-500 hover:text-gray-700">
                    Clear
                </a>
            @endif
        </form>
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-sm text-gray-500">{{ Str::limit($product->tagline, 40) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @if($product->sale_price && $product->sale_price < $product->price)
                                <div class="text-xs text-gray-500 line-through">৳{{ number_format($product->price, 0) }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <form action="{{ route('admin.products.toggle-publish', $product) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $product->is_published ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('products.show', $product) }}" target="_blank" class="text-gray-400 hover:text-gray-600" title="View">
