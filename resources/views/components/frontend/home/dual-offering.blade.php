...
<section class="py-20 lg:py-28 bg-neutral-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid lg:grid-cols-2 gap-8">
            {{-- Products Card --}}
            <div class="bg-white rounded-3xl p-8 lg:p-10 shadow-soft border border-neutral-100 hover:shadow-elevated transition-shadow">
                ...
                <h3 class="text-2xl font-bold text-neutral-900 mb-3">Digital Products</h3>
                ...
                @if($products->count() > 0)
                    <div class="space-y-4 mb-8">
                        @foreach($products->take(3) as $product)
                            <a href="{{ route('products.show', $product) }}" class="flex items-center gap-4 p-4 bg-neutral-50 rounded-xl hover:bg-primary-50 transition-colors group">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-neutral-900 group-hover:text-primary-600 truncate">{{ $product->name }}</h4>
                                    <p class="text-sm text-neutral-500 truncate">{{ $product->short_description }}</p>
                                </div>
                                @if($product->price)
            ...
            {{-- Services Card --}}
            <div class="bg-white rounded-3xl p-8 lg:p-10 shadow-soft border border-neutral-100 hover:shadow-elevated transition-shadow">
                <div class="w-14 h-14 bg-accent-100 rounded-2xl flex items-center justify-center mb-6">
                    <svg class="w-7 h-7 text-accent-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/>
                    </svg>
                </div>
                <h3 class="text-2xl font-bold text-neutral-900 mb-3">Custom Development</h3>
                <p class="text-neutral-600 mb-8">
                ...
                @if($services->count() > 0)
                    <div class="space-y-4 mb-8">
                        @foreach($services->take(4) as $service)
                            <a href="{{ route('services.show', $service) }}" class="flex items-center gap-4 p-4 bg-neutral-50 rounded-xl hover:bg-accent-50 transition-colors group">
                                ...
                                <div>
                                    <h4 class="font-semibold text-neutral-900 group-hover:text-accent-600">{{ $service->name }}</h4>
                                    <p class="text-sm text-neutral-500">{{ $service->short_description }}</p>
                                </div>
                            </a>
                        @endforeach
                    </div>
