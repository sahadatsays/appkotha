            ...
            @forelse($caseStudies as $study)
                <a href="{{ route('portfolio.show', $study) }}" class="group bg-white dark:bg-neutral-800 rounded-2xl overflow-hidden shadow-soft hover:shadow-elevated transition-shadow">
                    ...
                    <div class="p-6 lg:p-8">
                        ...
                        <h3 class="text-xl font-bold text-neutral-900 dark:text-white mb-3 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                            {{ $study->title }}
                        </h3>
                        ...
                        @if($study->metrics && count($study->metrics) > 0)
                            <div class="flex gap-6 pt-4 border-t border-neutral-100 dark:border-neutral-700">
                                @foreach(array_slice($study->metrics, 0, 3) as $metric)
                                    <div>
                                        <p class="text-lg font-bold text-primary-600 dark:text-primary-400">{{ $metric['value'] ?? '' }}</p>
                ...
                @for($i = 0; $i < 2; $i++)
                    <div class="group bg-white dark:bg-neutral-800 rounded-2xl overflow-hidden shadow-soft">
                        ...
                        <div class="p-6 lg:p-8">
                            ...
                            <div class="flex gap-6 pt-4 border-t border-neutral-100 dark:border-neutral-700">
                                <div>
                                    <p class="text-lg font-bold text-primary-600 dark:text-primary-400">{{ ['300%', '100K+'][$i] }}</p>
                                    <p class="text-xs text-neutral-500 dark:text-neutral-400">{{ ['Revenue Growth', 'Active Users'][$i] }}</p>
                                </div>
                                <div>
                                    <p class="text-lg font-bold text-primary-600 dark:text-primary-400">{{ ['50+', '99.9%'][$i] }}</p>
                                    <p class="text-xs text-neutral-500 dark:text-neutral-400">{{ ['Vendors', 'Uptime'][$i] }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
