                ...
                <div class="lg:col-span-3">
                        ...
                        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                            @foreach($posts as $post)
                                <a href="{{ route('blog.show', $post) }}" class="group">
                                    ...
                                    @endif
                                    <h3 class="text-lg font-bold text-neutral-900 mt-2 mb-2 group-hover:text-primary-600 transition-colors">
                                        {{ $post->title }}
                                    </h3>
                    ...
                    @else
                        <div class="text-center py-20">
                            ...
                            <h3 class="text-2xl font-bold text-neutral-900 mb-4">No Posts Yet</h3>
                            <p class="text-neutral-600">Check back soon for articles and insights.</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="lg:col-span-1">
                    ...
                    @if($categories->count() > 0)
                        <div class="bg-neutral-50 rounded-xl p-6 mb-8">
                            <h3 class="font-bold text-neutral-900 mb-4">Categories</h3>
                            <ul class="space-y-3">
                                @foreach($categories as $category)
                                    <li>
                                        <a href="{{ route('blog.category', $category) }}" class="flex items-center justify-between text-neutral-600 hover:text-primary-600 transition-colors">
                                            <span>{{ $category->name }}</span>
                                            <span class="text-sm text-neutral-400">({{ $category->posts_count }})</span>
                                        </a>
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    @if($featuredPosts->count() > 0)
                        <div class="bg-neutral-50 rounded-xl p-6">
                            <h3 class="font-bold text-neutral-900 mb-4">Popular Posts</h3>
                            <ul class="space-y-4">
                                @foreach($featuredPosts as $popular)
                                    <li>
                                        <a href="{{ route('blog.show', $popular) }}" class="text-sm text-neutral-700 hover:text-primary-600 transition-colors">
                                            {{ $popular->title }}
                                        </a>
                                    </li>
                                @endforeach
