    ...
    <article>
        ...
        <header class="py-16 bg-gradient-to-br from-neutral-50 via-white to-primary-50 dark:from-neutral-900 dark:via-neutral-900 dark:to-neutral-800">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <a href="{{ route('blog.index') }}" class="inline-flex items-center gap-2 text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 mb-6 transition-all duration-300 hover:translate-x-[-4px]" data-aos="fade-right" data-aos-duration="600">
                ...
                @if($post->category)
                    <a href="{{ route('blog.category', $post->category) }}" class="inline-block px-3 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 text-sm font-medium rounded-full mb-4 hover:bg-primary-200 dark:hover:bg-primary-900/50 transition-all duration-300 hover:scale-105" data-aos="fade-up" data-aos-delay="100">
                        {{ $post->category->name }}
                    </a>
                @endif

                <h1 class="text-4xl lg:text-5xl font-bold text-neutral-900 dark:text-white mb-6 leading-tight" data-aos="fade-up" data-aos-delay="150">
                    {{ $post->title }}
                </h1>

                <div class="flex items-center gap-6 text-neutral-600 dark:text-neutral-400" data-aos="fade-up" data-aos-delay="200">
                    @if($post->author)
        ...
        @if($post->featured_image)
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-4" data-aos="zoom-in" data-aos-delay="250">
                <img src="{{ asset('storage/' . $post->featured_image) }}" alt="{{ $post->title }}" class="w-full aspect-video object-cover rounded-2xl shadow-elevated hover:shadow-2xl transition-shadow duration-500">
            </div>
        @endif
    ...
    @if($relatedPosts->count() > 0)
        <section class="py-20 bg-neutral-50 dark:bg-neutral-800/50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h2 class="text-2xl font-bold text-neutral-900 dark:text-white mb-8" data-aos="fade-up">Related Articles</h2>
                <div class="grid md:grid-cols-3 gap-8">
                    @foreach($relatedPosts as $index => $related)
                        <a href="{{ route('blog.show', $related) }}" class="group hover-lift" data-aos="fade-up" data-aos-delay="{{ ($index % 3) * 100 }}">
                            <div class="aspect-video bg-neutral-200 dark:bg-neutral-700 rounded-xl mb-4 overflow-hidden img-zoom">
