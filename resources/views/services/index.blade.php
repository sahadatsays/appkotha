        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($services->count() > 0)
                <div class="grid md:grid-cols-2 gap-8">
                    @foreach($services as $service)
                        <a href="{{ route('services.show', $service) }}" class="group bg-white rounded-2xl p-8 border border-neutral-200 hover:border-primary-500 hover:shadow-elevated transition-all">
                            ...
                            @if($service->starting_price)
                                <p class="text-sm text-neutral-500">
                                    Starting from <span class="text-neutral-900 font-semibold">${{ number_format($service->starting_price, 0) }}</span>
                                </p>
                            @endif
                        </a>
                    @endforeach
            ...
            @else
                <div class="text-center py-20">
                    ...
                    <h3 class="text-2xl font-bold text-neutral-900 mb-4">Our Services</h3>
