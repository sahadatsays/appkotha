<x-layouts.frontend :title="$caseStudy->title">
    ...
    <section class="py-20 lg:py-28 bg-gradient-to-br from-neutral-50 via-white to-primary-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-3xl">
                <a href="{{ route('portfolio') }}" class="inline-flex items-center gap-2 text-primary-600 hover:text-primary-700 mb-6">
                ...
                @if($caseStudy->client_industry)
                    <span class="inline-block px-3 py-1 bg-primary-100 text-primary-600 text-sm font-medium rounded-full mb-4">
    ...
    <section class="pb-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="aspect-video bg-neutral-200 rounded-2xl flex items-center justify-center overflow-hidden">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid gap-16">
                ...
                @if($caseStudy->challenge)
                    <div>
                        <h2 class="text-2xl font-bold text-neutral-900 mb-6">The Challenge</h2>
                ...
                @if($caseStudy->solution)
                    <div>
                        <h2 class="text-2xl font-bold text-neutral-900 mb-6">Our Solution</h2>
                        <div class="prose prose-lg text-neutral-600">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-3">
                            @foreach($caseStudy->tech_stack as $tech)
                                <span class="px-4 py-2 bg-neutral-100 text-neutral-700 rounded-lg text-sm font-medium">
                                    {{ $tech }}
                                </span>
                            @endforeach
                        </div>
                ...
                @if($caseStudy->metrics && count($caseStudy->metrics) > 0)
                    <div>
                        <h2 class="text-2xl font-bold text-neutral-900 mb-6">Results</h2>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                            @foreach($caseStudy->metrics as $metric)
                                <div class="bg-primary-50 rounded-xl p-6 text-center">
                                    ...
                                    <div class="text-sm text-neutral-600">{{ $metric['label'] ?? '' }}</div>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                @if($caseStudy->testimonial_quote)
                    <div class="bg-neutral-900 rounded-2xl p-8">
                        <svg class="w-10 h-10 text-neutral-700 mb-4" fill="currentColor" viewBox="0 0 24 24">
    ...
    @if($relatedStudies->count() > 0)
        <section class="py-20 bg-neutral-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h2 class="text-2xl font-bold text-neutral-900 mb-8">More Projects</h2>
                <div class="grid md:grid-cols-3 gap-8">
                    @foreach($relatedStudies as $study)
                        <a href="{{ route('portfolio.show', $study) }}" class="group bg-white rounded-2xl overflow-hidden shadow-soft hover:shadow-elevated transition-shadow">
                            ...
                            <div class="p-6">
                                <h3 class="font-bold text-neutral-900 group-hover:text-primary-600 transition-colors">{{ $study->title }}</h3>
                            </div>
                        </a>
                    @endforeach
                </div>
